...
export function wp(width) {
    ...
    return PixelRatio.roundToNearestPixel(Dimension.viewportWidth * elemWidth);
}
...
export function hp(height) {
    ...
    return PixelRatio.roundToNearestPixel(Dimension.viewportHeight * elemHeight);
}
export const getAdjustSize = ({ width = 1, height = 1 }) => {
    const aspectRatio = width / height;
    const adjustWidth = wp(width);
    const adjustHeight = adjustWidth / aspectRatio;
    return { width: adjustWidth, height: adjustHeight };
};
